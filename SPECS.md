...
settings (
  ...
  value jsonb                          --   'min_match_stake'=100 (mínimo obrigatório por jogo; o resto expira)
                                        --   'house_stake'=50 (aposta da casa nos mercados normais, ver §4.3)
                                        --   'house_stake_exact'=200 (aposta da casa no Resultado exato × nº apostas, ver §4.3)
)
...
n_apostas    = COUNT(*) de todas as apostas do mercado

se pote_vencedor > 0:
  se mercado = 'Resultado exato':
    casa = settings('house_stake_exact') * n_apostas   -- por defeito 200 × nº de apostas
  senão se pote_vencedor = pote_total:
    casa = settings('house_stake')                     -- por defeito 50
  senão:
    casa = 0                                            -- já há mistura de vencedores/perdedores
  pote_pago = pote_total + casa   -- bónus da casa, não preso a nenhuma opção
...
```
**Aposta da casa** — a casa não aposta numa opção específica; mete sempre fichas extra no pote a
dividir por quem acertar. Duas regras:
- **Mercados normais**: `settings('house_stake')` (por defeito 50) só entra quando
  `pote_vencedor = pote_total` — ou seja, só apostou uma pessoa (e acertou) ou toda a gente
  apostou no mesmo palpite (e saiu certo). Sem a casa, esses vencedores limitar-se-iam a reaver o
  que puseram — um "reembolso" disfarçado. Quando há mistura real de vencedores e perdedores, os
  vencedores já lucram à custa de quem perdeu — a casa não entra.
- **"Resultado exato"** (risco alto, raro de acertar): `settings('house_stake_exact')` (por
  defeito 200) entra **sempre** que há vencedor, multiplicado pelo nº de apostas feitas no
  mercado — o Jackpot cresce com a malta a jogar.

A casa não tem saldo nem transação própria; o bónus só aparece dentro do `payout` de quem ganhou.
Põe as settings a `0` para desligar cada regra.
...
9. **Aposta da casa**: fichas extra no pote de quem acerta, nunca presas a uma opção, nunca pagas
   se ninguém acertar. Nos mercados normais só entra (`house_stake`, 50) quando só apostou 1
   pessoa ou toda a gente apostou no mesmo palpite — para nunca ser um "reembolso" disfarçado. No
   "Resultado exato" entra sempre (`house_stake_exact`, 200 × nº de apostas) — o Jackpot cresce
   com a malta. Ver §4.3.
